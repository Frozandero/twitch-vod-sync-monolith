...
- Balance the grid by recording count and available width: four recordings use 2×2 when space permits, shorter rows fill the width, and small screens stack players. Fit sessions of four or more to available viewport height while retaining Twitch's minimum player size and scrolling when necessary. Layout changes must only update CSS positions/sizes, never remount players or issue playback commands. Watch-mode exit and timeline toggle belong in the last header of the first row, with no empty control strips or overlap over video.
