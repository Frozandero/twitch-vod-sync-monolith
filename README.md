...
6. **Watch mode** hides setup controls and fills the available height with the responsive grid. Its exit icon sits beside the timeline toggle in the top-right player header; Escape also restores setup. Neither control reserves an empty row or covers a video. Each player has sync, mute, settings, open, and remove controls. **Undo** restores the last closed recording to its former position. Closing a non-source recording keeps the others playing when the selected moment still fits the remaining timeline.
...
The grid groups streams automatically. On a wide desktop, four use 2×2, five use 3+2, and six use 3×2; shorter rows fill the available width. Up to three streams stay in one row when space permits. Sessions of four or more share the remaining window height, with scrolling when necessary to keep players usable. Resizing, reordering, and toggling watch mode keep the same players mounted. Narrow screens stack them vertically.
