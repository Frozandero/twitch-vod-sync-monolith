...
Watch mode measures the available workspace with ResizeObserver and chooses the largest equal video widths that fit its rows, accounting for player headers, messages, gaps, and Twitch's minimum 400×300 frame. Frames use 16:9 proportions with that minimum height instead of stretching to fill tall columns. Flex wrapping centers incomplete rows while retaining CSS order and the original iframe DOM nodes. Timeline collapse, viewport resize, and recording count changes recompute the fit without playback commands. When minimum sizes cannot fit, the workspace scrolls; it never crops or scales down Twitch's controls. The regular grid remains unchanged.
